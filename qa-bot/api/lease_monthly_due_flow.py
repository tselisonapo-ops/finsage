...
class LeaseMonthlyDueFlow(BaseFlow):
    ...
    def run(self) -> dict:
        ...
        res = self.client.get(
            ROUTES["lease_monthly_due"].format(company_id=self.company_id),
            params=params,
        )
        assert_http_ok(res.status_code, res.text)

        data = self.client.safe_json(res) or {}
        assert_true(isinstance(data, dict), f"Lease monthly due response was not JSON. Body: {res.text[:500]}")
        assert_true(data.get("ok") is True, f"Lease monthly due failed. Response: {data}")
