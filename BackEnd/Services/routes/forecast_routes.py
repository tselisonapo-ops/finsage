...
@forecast_bp.route("/api/companies/<int:company_id>/forecast/budgets/<int:budget_id>/lines", methods=["POST", "PUT", "OPTIONS"])
@require_auth
def forecast_budget_lines_upsert(company_id, budget_id):
    if request.method == "OPTIONS":
        return _opt()
    ...
    try:
        ...
        if isinstance(body.get("lines"), list):
            out = db_service.forecast_bulk_upsert_budget_lines(
                company_id,
                budget_id,
                body["lines"],
                user_id=_actor_user_id(payload),
            ...
            out = db_service.forecast_upsert_budget_line(
                company_id,
                budget_id,
                body,
                user_id=_actor_user_id(payload),
        ...
        current_app.logger.exception("forecast upsert budget lines failed")
        return _json_error(e, 400)
