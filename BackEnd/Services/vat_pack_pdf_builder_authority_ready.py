...
def generate_vat_return_pdf(filing: dict, company: dict, *, start_date, end_date, authority_return: dict = None) -> bytes:
    filing = filing or {}
    company = company or {}
    ...
    authority_return = authority_return or filing.get("authority_payload") or {}
    if authority_return:
        authority_code = _safe(authority_return.get("authority_code")) or "-"
        form_code = _safe(authority_return.get("form_code")) or "-"
        spec_version = _safe(authority_return.get("spec_version")) or "-"
        validation_status = _safe(authority_return.get("validation_status")) or "-"
        fields = authority_return.get("fields") or authority_return.get("boxes") or {}

        authority_rows = [[
            Paragraph("Return Box / Field", styles["FS_Label"]),
            Paragraph("Amount", styles["FS_RightBold"]),
        ]]
        for key, value in fields.items():
            authority_rows.append([
                Paragraph(_safe(key), styles["FS_Body"]),
                Paragraph(_fmt_currency(value, currency), styles["FS_Right"]),
            ])

        meta = [
            [Paragraph("Revenue Authority", styles["FS_Label"]), Paragraph(authority_code, styles["FS_BodyBold"])],
            [Paragraph("Official Form", styles["FS_Label"]), Paragraph(form_code, styles["FS_Body"])],
            [Paragraph("Specification", styles["FS_Label"]), Paragraph(spec_version, styles["FS_Body"])],
            [Paragraph("Validation", styles["FS_Label"]), Paragraph(validation_status.upper(), styles["FS_BodyBold"])],
        ]
        story.append(Paragraph("Authority Filing Map", styles["FS_SectionTitle"]))
        story.append(_section_table(meta, [content_w * 0.36, content_w * 0.64]))
        story.append(Spacer(1, 4 * mm))
        story.append(_section_table(authority_rows, [content_w * 0.65, content_w * 0.35], header=True))
        story.append(Spacer(1, 7 * mm))

        errors = authority_return.get("validation_errors") or []
        if errors:
            error_rows = [[
                Paragraph("Validation Code", styles["FS_Label"]),
                Paragraph("Issue", styles["FS_Label"]),
            ]]
            for error in errors:
                error_rows.append([
                    Paragraph(_safe(error.get("code")), styles["FS_Body"]),
                    Paragraph(_safe(error.get("message")), styles["FS_Body"]),
                ])
            story.append(Paragraph("Filing Validation", styles["FS_SectionTitle"]))
            story.append(_section_table(error_rows, [content_w * 0.28, content_w * 0.72], header=True))
            story.append(Spacer(1, 7 * mm))
