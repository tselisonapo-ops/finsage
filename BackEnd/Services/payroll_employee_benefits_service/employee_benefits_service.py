...
class PayrollEmployeeBenefitsService:
    ...
    def disclosure(self, company_id: int, reporting_date: str) -> dict:
        ...
        leave = self.db.fetch_one(f"""
            SELECT
              COALESCE(SUM(opening_provision),0) AS opening,
              COALESCE(SUM(provision_movement),0) AS movement,
              COALESCE(SUM(closing_provision),0) AS closing
            FROM {schema}.payroll_employee_leave_balances
            WHERE company_id=%s AND as_of_date=%s;
        """, (int(company_id), reporting_date)) or {}
        valuations = self.actuarial_valuations_list(company_id)
        return {
            "reporting_date": reporting_date,
            "dashboard": dash,
            "leave_reconciliation": {
                "opening": money(leave.get("opening")),
                "movement": money(leave.get("movement")),
                "closing": money(leave.get("closing")),
            },
            "defined_benefit_valuations": [
                v for v in valuations if str(v.get("valuation_date")) <= reporting_date
            ],
            "note": (
                "Defined-benefit measurements are based on imported or approved "
                "actuarial valuation data retained by the entity."
            ),
        }
