...
@app.route("/api/companies/<int:company_id>/invoices/<int:invoice_id>/receipt/email", methods=["POST"])
@require_auth
def email_receipt_by_invoice(company_id: int, invoice_id: int):
    ...
    # You need a way to fetch the receipt linked to this invoice
    rcpt = db_service.fetch_one(
        f"""
        SELECT r.*
        FROM {db_service.company_schema(company_id)}.receipts r
        JOIN {db_service.company_schema(company_id)}.receipt_allocations ra
...
@app.route("/api/companies/<int:company_id>/receipts/<int:receipt_id>/email", methods=["POST"])
@require_auth
def email_receipt(company_id: int, receipt_id: int):
    ...
    to_email = (
        preferred
        or rcpt.get("customer_email")
        or rcpt.get("company_email")
    )
    if not to_email:
        return jsonify({"error": "Customer has no email."}), 400
